	...
	this.getSelecteds = function(field) {
		...
		$table.find(".group-checkable-item").each(function(i, item) {
			if ($(item).prop('checked')) {
				var data = target.page[target.opts.dataName][$(item).val()];
				if(field){
					arr.push(data[field]);
				}else{
					arr.push(data);
...
//		var $table = $(this.opts.id);
	...
	this.getRowHtml = function(row,index){
		var bodyHtml = '<tr role="row" class="gradeX odd">';
		for ( var j = 0; j < opts.columns.length; j++) {
			...
			if (col.checkbox) {
				bodyHtml += '<td><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"> <input class="group-checkable-item" value="'+ index+ '" type="checkbox"> <span></span></label></td>';
			} else {
				...
				bodyHtml += '<td class="' + className+ '">' + val + '</td>';
			}
		}
		bodyHtml += '</tr>';
		return bodyHtml;
	...
	this.getHeadHtml = function(){
		...
		for ( var i = 0; i < opts.columns.length; i++) {
			...
			if (col.checkbox) {
				...
				if(sort){
					headHtml += '<th class="sorting ' + className + '" sort="'+sort+'">' + title + '</th>';
				}else{
					headHtml += '<th class="' + className + '">' + title + '</th>';
				}
				//sorting_desc
			}
		}
		...
		$.ajax({
			...
			success : function(page) {
				if(page){
					...
					target.page = page;
					var data = target.page[target.opts.dataName];
